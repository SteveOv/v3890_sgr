...
for spec_group in ["target_observation", "standard_observation"]:
    ...
    plot_nss_spectra = read_setting(group_settings, "plot_nss_spectra", False)
    ...
    for fits_file_name in sorted(pathlib.Path.home().glob(read_setting(group_settings, "source_files", "")), key=os.path.basename):
        ...
        non_ss_spectra, header = FrodoSpecDS.read_spectra(fits_file_name, "RSS_NONSS", header=True, label=basename)
        ...
        man_exclusion_mask[read_setting(sub_settings, "excluded_fibres", [])] = False
        ...
        obj_spike_removal = read_setting(sub_settings, "spike_removal", None)
        ...
        if plot_nss_spectra:
            plotting.plot_rss_spectra(non_ss_spectra, flux_ratios, basename,
